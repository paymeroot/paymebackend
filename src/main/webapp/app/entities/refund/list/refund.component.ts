...
import { EntityArrayResponseType, RefundService } from '../service/refund.service';
...
export class RefundComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['reference', 'transactionRef', 'refundStatus'];
  ...
  trackId = (_index: number, item: IRefund): number => this.refundService.getRefundIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && RefundComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && RefundComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.refunds = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IRefund[] | null): IRefund[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.refundService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.refundService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
